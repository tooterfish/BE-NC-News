...
exports.fetchArticles = (topic, sortBy, order, limit = 10, page = 1) => {
  ...
  const queryStr = `
  SELECT COUNT(articles.*) OVER()::int AS total_count, articles.author, articles.title, articles.article_id, articles.topic, articles.created_at, articles.votes, articles.article_img_url, COUNT(comment_id)::int AS comment_count FROM articles
